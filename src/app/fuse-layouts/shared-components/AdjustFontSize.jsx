...
function AdjustFontSize(props) {
    ...
    const font = useSelector(({ auth }) => auth.user.data?.user_personalization?.font_size || 1);
    ...
    const handleClose = () => {
        setAnchorEl(null);
        const newUser = { ...user, user_personalization: { ...user.user_personalization, font_size: fontSize } };
        ...
        if (user?.reseller_id) JwtService.updateDataToSessionStorage(newUser)
            ...
            dispatch(updateUserPersonalSetting({ font_size: fontSize }))
        }
